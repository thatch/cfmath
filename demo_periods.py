...
from cfmath import CF, Sqrt
from cfmath.gosper import cf_mul, _MAX_STALL

def alarm(s, f): raise TimeoutError()
...
def try_mul(a, b, label):
    ...
    print("  sqrt(%2d) x sqrt(%2d): periods (%d,%d) gcd=%d  same-field=%s  -> %s  %s" % (
        a, b, pa, pb, math.gcd(pa,pb),
        'YES' if same_field else 'NO ',
        result,
        '(coprime, BUT same-field!)' if (coprime_periods and same_field and terms is None)
        else '(coprime, diff-field)' if (coprime_periods and not same_field)
        else '(equal periods, same-field)' if (not coprime_periods and same_field)
        else ''))
